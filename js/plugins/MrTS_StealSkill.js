	    	...
	    	if (target.result().stolenItems.length > 0)
	    	{
		    	...
		    	for (var i = 0; i < stolenItems.length; i++)
		    	{
		    		if (!DataManager.isSkill(stolenItems[i])){
                                                                        var stealResult = true;
		    			this.push('addStealText', stealSuccessText.format(subject.name(), stolenItems[i].name, target.name()));
		    		}else{
                                                                        var stealResult = true;
		    			this.push('addStealText', stealSkillSuccessText.format(subject.name(), stolenItems[i].name, target.name()));
                                                          }
		    	}
		    }
	        if (target.result().noItemsToSteal && target.result().stealAttempt){
	        	if(stealResult == false)this.push('addStealText', nothingLeftText.format(target.name()));
	        }else if (target.result().failedSteal){
	        	if(stealResult == false)this.push('addStealText', stealFailText.format(subject.name(), target.name()));
                       }
	    }
	};

Window_BattleLog.prototype.addStealText = function(text) {
