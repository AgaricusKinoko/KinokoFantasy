...
Game_Action.prototype.calcElementRate = function(target) {
    ...
    if (this.item().damage.elementId < 0) {
        if(this.item().damage.variance != 0 && this.elementsMaxRate(target, this.subject().attackElements()) > 1) weak = true;
        if(this.item().damage.variance != 0 && this.elementsMaxRate(target, this.subject().attackElements()) < 1) regist = true;
        return this.elementsMaxRate(target, this.subject().attackElements());
    } else {
        if(this.item().damage.variance != 0 && target.elementRate(this.item().damage.elementId) > 1) weak = true;
        if(this.item().damage.variance != 0 && target.elementRate(this.item().damage.elementId) < 1) regist = true;
