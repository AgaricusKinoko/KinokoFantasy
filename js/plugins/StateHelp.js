...
(function(){
	...
	Window_StateHelp.prototype.drawItem = function(index) {
		...
		for (var i=this._topCol;i<(c+this._topCol);i++){
			...
			this.drawIcon(character.allIcons()[i],x+iw*ii+134,y+(iw-32)/2, character, character.states()[i], i);
		}
	};

	var decimalPlaces = 0;
	var turnFontSize = 16;
	var turnTextPosition = 'topright';
	var _Window_Base_drawIcon = Window_Base.prototype.drawIcon;
	Window_Base.prototype.drawIcon = function (iconIndex, x, y, actor, state, index) {
			...
			if(actor){
				...
				if (state.autoRemovalTiming != 0 && state.iconIndex > 0) {
						turns = actor._stateTurns[actor._states[index]];
				}
				if(state.autoRemovalTiming == 0){
					turns = -1;
				}
				if(actor._stackState){
						actor._stackState.forEach(function(stack){
							if(stack.id == state.id){
								stacks = stack.stackCount;
							}
						})
				}
				...
				if(turns != -1){
					...
					this.drawText(turns.toFixed(decimalPlaces) + "t", this.turnsRemainingPosX(0,x) - 4, this.turnsRemainingPosY(y) + 18, Window_Base._iconWidth, 'center');
				}
				this.changePaintOpacity(true);
				if(stacks && stacks != 0){
					this.changeTextColor(this.crisisColor());
					this.drawText(stacks, this.stacksDisplayPosX(0,x), this.stacksDisplayPosY(y), Window_Base._iconWidth, 'center');
				}
