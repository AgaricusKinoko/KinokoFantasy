 ...
 */
 ...
 /*:ja
 * @plugindesc キーバインドをプレイヤーが変更出来るようにします。
 * @author Yanfly Engine Plugins
 *
 * @param Command Name
 ...
 *
 * @param Key Help
 * @desc 設定画面のヘルプメッセージです。
 * 記入した文章は、画面上部のヘルプウィンドウに描画されます。
 * @default どのキーバインドを変更しますか？
 *
 * @param Default Layout
 * @desc 初期化コマンドの名前です。
 * @default 初期設定
 ...
 * 記入した文章は、画面上部のヘルプウィンドウに描画されます。
 * @default キーボードの設定を初期状態に戻します。
 *
 * @param WASD Layout
 * @desc WASD移動コマンドの名前です。
 * （ＦＰＳなどでよく使われるアレです。）
 * @default WASD移動
 *
 * @param WASD Help
 * @desc WASD移動コマンドの役割を説明する場合、ここに記入します。
 * 記入した文章は、画面上部のヘルプウィンドウに描画されます。
 * @default キーボードの設定をWASD移動に適した設定に変更します。
 *
 * @param Finish Config
 * @desc 設定完了コマンドの名前です。
 * @default 設定完了
 ...
 * 記入した文章は、画面上部のヘルプウィンドウに描画されます。
 * @default キーバインドの設定を完了します。
 *
 * @param Assigned Color
 * @desc キーバインドが設定されているキーの背景色です。
 * （使用出来る色と数字は、メッセージの表示の「\C[]」で使用出来る色と同一です。
 * @default 21
 *
 * @param Action Color
 * @desc 設定された機能の名前を表示する文字色です。
 * （使用出来る色と数字は、メッセージの表示の「\C[]」で使用出来る色と同一です。
 * @default 4
 *
 * @param Clear Text
 * @desc 設定の解除を行うコマンドの名前です。
 * @default 設定を解除
 *
 * @param OK Key
 * @desc 決定キーの名前です。
 * （設定した名前は、キーの下に小さく表示されるため、長いと潰れます。）
 * @default 決定
 *
 * @param OK Text
 * @desc 決定キーを設定するコマンドの名前です。
 * @default 決定／話しかける
 *
 * @param Escape Key
 * @desc キャンセル／メニューキーの名前です。
 * （設定した名前は、キーの下に小さく表示されるため、長いと潰れます。）
 * @default C/M
 *
 * @param Escape Text
 * @desc キャンセル／メニューキーを設定するコマンドの名前です。
 * @default キャンセル／メニュー
 *
 * @param Cancel Key
 * @desc キャンセルキーの名前です。
 ...
 *
 * @param Down Text
 * @desc 下移動キーを設定するコマンドの名前です。
 * @default 下に移動する
 *
 * @help
 * このプラグインは、オプションに「キーバインドの設定」を追加します。
 * ただし、この設定は、パソコンでプレイすることを前提としていますので、
 * AndroidやiPhoneといった、モバイル端末でプレイする場合、設定は適用されません。
 * キーバインド設定画面では、キーボードのレイアウトが表示されるので、
 * プレイヤーはそれを見ながらキーバインドを変更する事が出来ます。
 *
 * ただし、動作不能になってしまうことを防ぐため、
 * Enterキー、および矢印キーのキーバインドを変更する事は出来ません。
 * それ以外のキーは、ほぼ全て、プレイヤーの好きなように変更する事が出来ます。
 *
 * ※キノコ訳の当プラグインの追加要素として、
...
if(Yanfly.Param.KeyConfigKeyHelp.indexOf("\\n") >= 0){
	var ntext = Yanfly.Param.KeyConfigKeyHelp.indexOf("\\n");
	var ntext1 = Yanfly.Param.KeyConfigKeyHelp.substr(0,ntext);
	var ntext2 = Yanfly.Param.KeyConfigKeyHelp.substr(ntext);
	Yanfly.Param.KeyConfigKeyHelp = ntext1 + "\n" + ntext2;
}
...
if(Yanfly.Param.KeyConfigDefaultHelp.indexOf("\\n") >= 0){
	var ntext = Yanfly.Param.KeyConfigDefaultHelp.indexOf("\\n");
	var ntext1 = Yanfly.Param.KeyConfigDefaultHelp.substr(0,ntext);
	var ntext2 = Yanfly.Param.KeyConfigDefaultHelp.substr(ntext);
	Yanfly.Param.KeyConfigDefaultHelp = ntext1 + "\n" + ntext2;
}
